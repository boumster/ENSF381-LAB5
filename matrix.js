...
function performOperation(operation) {
    ...
    if (result) showResult2D('The Result', 'matrix3', result); // use suitable function for printing results
}
...
function addMatrices(matrix1, matrix2) {
    if (matrix1.length !== matrix2.length || matrix1[0].length !== matrix2[0].length) {
        console.log("Cannot add matrices. Matrices must have the same dimensions.");
        return;
    }
...
const multiplyMatrices = (matrix1, matrix2) => {
    if (matrix1[0].length !== matrix2.length) {
        console.log("Cannot multiply matrices. The number of columns in the first matrix must be equal to the number of rows in the second matrix.");
        return;
    }
